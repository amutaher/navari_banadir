...
def sync_sequence(doc, method):
    ...
    if len(doc.po_items) != len(doc.sub_assembly_items):
        frappe.throw(
            f"The number of items in <span style='color:red';>'Assembly Items'</span> and <span style='color:red';>'Sub Assembly Items'</span> must be equal(<b>{len(doc.po_items)}</b>)"
        )
    ...
    for idx in range(len(doc.po_items)):
        ...
        frappe.db.set_value(
            ...
            doc.po_items[idx].name,
            "custom_seq_id",
            current_seq_id,
        )
        frappe.db.set_value(
            "Production Plan Sub Assembly Item",
            doc.sub_assembly_items[idx].name,
            "custom_seq_id",
            current_seq_id,
        )
...
def auto_name(doc, method=None):
    ...
    if doc.doctype == "Work Order":
        if doc.production_plan_item:
            doc.custom_seq_id = get_seq_id(
                doc.production_plan_item, "Production Plan Item"
            )
...
def get_seq_id(_item, doc):
    ...
    """
    seq_id = frappe.db.get_value(doc, _item, "custom_seq_id")
    return seq_id
...
def validate_finished_insole(doc):
    if not hasattr(doc, "po_items") or not hasattr(doc, "sub_assembly_items"):
        frappe.throw(
            "Both child tables (Production Plan Item and Sub Assembly Item) must exist"
        )

    if len(doc.po_items) != len(doc.sub_assembly_items):
        frappe.throw(
            f"The number of items in <span style='color:red';>'Finished Goods Items'</span> and <span style='color:red';>'Sub Assembly Items'</span> must be equal (<b>{len(doc.po_items)}</b>)"
        )
...
def create_production_split_items(original_item, split_result):
    ...
    for i in range(split_no):
        ...
        new_item = {
            **{
                field: original_item.get(field)
                for field in ["item_code", "bom_no", "stock_uom", "planned_start_date"]
            },
            "planned_qty": qty,
            "pending_qty": qty,
            "custom_split_no": 1,
...
def split_po_items():
    ...
    try:
        ...
        return {
            "success": True,
            "message": _("Production items split successfully"),
            "docname": doc.name,
        }
...
def split_prod_assembly():
    ...
    try:
        ...
        return {
            "success": True,
            "message": _("Sub-assembly items split successfully"),
            "docname": doc.name,
        }
    except Exception as e:
