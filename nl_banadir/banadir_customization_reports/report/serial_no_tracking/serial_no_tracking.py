...
from erpnext.stock.doctype.serial_no.serial_no import (
    get_serial_nos as get_serial_nos_from_sle,
)
from frappe.utils import get_time, getdate


BUYING_VOUCHER_TYPES = [
    "Purchase Invoice",
    "Purchase Receipt",
...
def get_data(filters):
    stock_ledgers = get_stock_ledger_entries(
        filters, "<=", order="asc", check_serial_no=False
    )
    ...
    for row in stock_ledgers:
        ...
        args.party = (
            frappe.db.get_value(row.voucher_type, row.voucher_no, party_field)
            if party_field
            else None
        )
                ...
                serial_nos.append(
                    {
                        "serial_no": serial_no,
                        "valuation_rate": abs(
                            row.stock_value_difference / row.actual_qty
                        ),
        ...
        if row.serial_and_batch_bundle:
            serial_nos.extend(
                bundle_wise_serial_nos.get(row.serial_and_batch_bundle, [])
            )
...
def get_serial_nos(filters, serial_bundle_ids):
    ...
    for d in frappe.get_all(
        ...
        engine_no = frappe.db.get_value(
            "Serial No", d.serial_no, "custom_engine_number"
        )
