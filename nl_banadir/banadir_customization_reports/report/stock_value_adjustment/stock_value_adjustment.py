...
def convert_as_per_current_exchange_rate(data, filters, from_currency, to_currency):
    ...
    for entry in data:
        ...
        if "rate" in entry and "exchange_rate" in entry:
            ...
            old_rate_plus_landed_cost_in_usd = (
                entry["rate_plus_landed_cost"] / entry["exchange_rate"]
            )
            old_landed_cost = (
                entry["landed_cost_voucher_amount"] / entry["exchange_rate"]
            )

            current_rate_chosen_currency = (
                get_current_exchange_rate(from_currency, to_currency, date)
                * old_rate_in_usd
            )
            current_rate_plus_landed_cost_chosen_currency = (
                get_current_exchange_rate(from_currency, to_currency, date)
                * old_rate_plus_landed_cost_in_usd
            )
            current_rate_in_usd = convert(
                current_rate_chosen_currency, "USD", to_currency, date
            )
            current_rate_plus_landed_cost_in_usd = convert(
                current_rate_plus_landed_cost_chosen_currency, "USD", to_currency, date
            )
            current_landed_cost_chosen_currency = (
                get_current_exchange_rate(from_currency, to_currency, date)
                * old_landed_cost
            )
            current_landed_cost_in_usd = convert(
                current_landed_cost_chosen_currency, "USD", to_currency, date
            )

            if filters.get("presentation_currency") == "USD":
                entry["current_rate"] = current_rate_in_usd
                entry["current_rate_plus_landed_cost"] = (
                    current_rate_plus_landed_cost_in_usd
                )
...
def _execute(filters=None, additional_table_columns=None):
    ...
    for d in item_list:
        ...
        expense_account = (
            d.unrealized_profit_loss_account
        ...
        row = {
            ...
            **get_values_for_columns(additional_table_columns, d),
            ...
            if d.stock_qty
            else d.base_net_amount,
            "amount": d.base_net_amount,
            "exchange_rate": d.conversion_rate
            if d.conversion_rate != 1
            else get_current_exchange_rate("USD", company_currency, d.posting_date),
            "landed_cost_voucher_amount": flt(
                d.landed_cost_voucher_amount / d.stock_qty
            ),
            "rate_plus_landed_cost": flt(d.base_net_amount / d.stock_qty)
            + flt(d.landed_cost_voucher_amount / d.stock_qty),
            "amount_plus_landed_cost": d.base_net_amount
...
def convert_currency_fields(data, filters):
    ...
    for entry in data:
        ...
        entry["amount"] = convert(
            entry.get("amount", 0), from_currency, to_currency, date
        )
        entry["landed_cost_voucher_amount"] = convert(
            entry.get("landed_cost_voucher_amount", 0), from_currency, to_currency, date
        )
        entry["rate_plus_landed_cost"] = convert(
            entry.get("rate_plus_landed_cost", 0), from_currency, to_currency, date
        )
        entry["total_tax"] = convert(
            entry.get("total_tax", 0), from_currency, to_currency, date
        )
        entry["total"] = convert(
            entry.get("total", 0), from_currency, to_currency, date
        )
        entry["amount_plus_landed_cost"] = convert(
            entry.get("amount_plus_landed_cost", 0), from_currency, to_currency, date
        )
        entry["total_landed_cost"] = convert(
            entry.get("total_landed_cost", 0), from_currency, to_currency, date
        )
        if filters.get("presentation_currency") != "USD":
            entry["current_rate"] = convert(
                entry.get("current_rate", 0), from_currency, to_currency, date
            )
            entry["current_rate_plus_landed_cost"] = convert(
                entry.get("current_rate_plus_landed_cost", 0),
                from_currency,
                to_currency,
                date,
            )
            entry["current_total"] = convert(
                entry.get("current_total", 0), from_currency, to_currency, date
            )
            entry["current_landed_cost"] = convert(
                entry.get("current_landed_cost", 0), from_currency, to_currency, date
            )
...
def get_opening_stock_qty(filters):
    ...
    opening_stock_map = {d["item_code"]: d["opening_qty"] for d in opening_stock_data}
        ...
        if item_code:
            if item_exists_map.get(item_code):
                ...
                for key in [
                    ...
                    "current_landed_cost",
                    "current_rate_plus_landed_cost",
                    "rate_plus_landed_cost",
                ]:
                    if key in row:
                        value = row[key]
                        if isinstance(value, (int, float)):
...
def append_total_row(data):
    ...
    total_row = {
        key: 0 for key in data[0].keys() if isinstance(data[0][key], (int, float))
    }
    total_row["bold"] = 1
