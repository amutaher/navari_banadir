...
def get_columns(filters):
    ...
    columns += [
        {
            "label": f"<strong>Total ({presentation_currency})</strong>",
            "fieldname": "total_difference_currency",
            "fieldtype": "Float",
            "precision": 2,
            "width": 120,
        },
...
def fetch_data(filters):
    ...
    account_monthly_differences, total_differences = calculate_differences_by_month(
        ...
        account_filter,
        hide_account_filter,
        hide_parent_account_filter,
    )

    if show_parent_accounts:
        # Aggregate data for parent accounts
        data = aggregate_parent_accounts(
            account_monthly_differences, total_differences, company
        )
    else:
        data = prepare_final_data(
            account_monthly_differences, total_differences, filters
        )
    return data


def calculate_differences_by_month(
    start_date,
    end_date,
    company,
    account_filter,
    ...
    while current_date <= end_date:
        ...
        monthly_sums = fetch_monthly_sums(
            company,
            month_start,
            next_month_start,
            account_filter,
