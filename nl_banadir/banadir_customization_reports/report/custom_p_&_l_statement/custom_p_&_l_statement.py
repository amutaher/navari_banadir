...
def execute(filters=None):
    ...
    data = get_data(
        company, from_date, to_date, filters, presentation_currency, periodicity
    )
...
def process_account_category(
    ...
    top_level_accounts = [
        acc
        for acc in accounts
        if acc.parent_account is None or acc.parent_account not in account_dict
    ]
...
def process_account_with_children(
    account,
    all_accounts,
    gl_entries,
    periods,
...
def get_profit_loss_chart(total_income, total_expense, profit, currency):
    return {
        "data": {
            ...
            "datasets": [{"values": [total_income, total_expense, profit]}],
        },
        "type": "bar",
        "colors": [
            "#28a745",
            "#dc3545",
            "#007bff",
        ],  # Green for Income, Red for Expense, Blue for Profit
...
def get_gl_entries_by_account(
    company, from_date, to_date, ignore_opening_entries=False
):
