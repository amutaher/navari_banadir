...
def get_purchase_invoice_items(project_name, company_filter=None):
    ...
    query = (
        ...
        .where(
            (PurchaseInvoice.project == project_name) & (PurchaseInvoice.docstatus == 1)
        )
    )
...
def get_data(filters):
    ...
    if not project_filter:
        ...
        projects = frappe.get_all(
            "Project", filters={"name": project_filter}, fields=["name"]
        )
    ...
    for project in projects:
        ...
        purchase_invoice_items = get_purchase_invoice_items(
            project_name, company_filter
        )
        ...
        for pi in purchase_invoice_items:
            ...
            if currency_filter and currency_filter != pi.currency:
                transaction_date = (
                    pi.posting_date if pi.posting_date else frappe.utils.nowdate()
                )

                pi.rate = convert_currency(
                    pi.rate, pi.currency, currency_filter, transaction_date
                )
                pi.amount = convert_currency(
                    pi.amount, pi.currency, currency_filter, transaction_date
                )
        ...
        for se in stock_entry_details:
            ...
            if currency_filter and currency_filter != se.currency:
                transaction_date = (
                    se.posting_date if se.posting_date else frappe.utils.nowdate()
                )

                se.rate = convert_currency(
                    se.rate, se.currency, currency_filter, transaction_date
                )
                se.amount = convert_currency(
                    se.amount, se.currency, currency_filter, transaction_date
                )
        ...
        for i in range(max_length):
            ...
            if i < len(item_code_list):
                ...
                purchased_amount = purchase_data.get(item_code, {}).get(
                    "purchased_amount", 0
                )
                uom = purchase_data.get(item_code, {}).get(
                    "uom", stock_data.get(item_code, {}).get("uom", "")
                )
                currency_symbol = purchase_data.get(item_code, {}).get(
                    "currency_symbol", ""
                )

                consumed_qty = stock_data.get(item_code, {}).get("consumed_qty", 0)
                stock_rate = stock_data.get(item_code, {}).get("stock_rate", 0)
                consumed_amount = stock_data.get(item_code, {}).get(
                    "consumed_amount", 0
                )
            ...
            data.append(
                {
                    ...
                    "stock_rate": f"{currency_symbol} {stock_rate:,.2f}"
                    if stock_rate
                    else "",
                    ...
                    "name": name,
                    "task_status": f"{format_task_status(task_status)}"
                    if task_status
                    else "",
