...
def get_columns(filters):
    ...
    return [
        ...
        },
        {
            ...
            "fieldname": "qty",
            "fieldtype": "Int" if filters.remove_precision == 1 else "Float",
            "width": 120,
        },
        {
            "label": "Current Qty in Destination",
...
def calculate_subtotals(data, filters):
    ...
    for row in data:
        if current_stock_entry and current_stock_entry != row["stock_entry"]:
            ...
            total_value = (
                frappe.db.get_value(
                    "Stock Entry", current_stock_entry, "total_outgoing_value"
                )
                or 0
            )
            formatted_data.append(
        ...
        formatted_data.append(row)
        subtotal_qty += row.get("qty", 0)
        subtotal_current_qty += (
            row.get("current_qty", 0) if row.get("current_qty") is not None else 0
        )
        current_stock_entry = row["stock_entry"]
...
def get_data(filters):
    ...
    formatted_data, total_qty, total_current_qty, total_amount = calculate_subtotals(
        data, filters
    )
