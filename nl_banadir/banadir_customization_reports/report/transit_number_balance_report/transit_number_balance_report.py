...
def get_columns(filters: dict | None = None) -> list[dict]:
    columns = [
        ...
        {"fieldname": "uom", "label": "Item UOM", "fieldtype": "Data", "width": 120},
        {
            "fieldname": "qty_in",
            "label": "Total Qty In",
            "fieldtype": "Float",
...
def get_data(filters) -> list[list]:
    ...
    query = (
        ...
        .select(
            ...
            Round(pii_agg.total_qty_in - Coalesce(sii_agg.total_qty_out, 0), 2).as_(
                "balance"
            ),
            Round(Coalesce(pii_agg.avg_purchase_amount, 0), 2).as_(
    ...
    for row in data:
        ...
        if row.get("sales_invoices"):
            sales_invoice_links = ", ".join(
                [
                    f'<a href="/app/sales-invoice/{inv.strip()}" target="_blank">{inv.strip()}</a>'
                    for inv in row["sales_invoices"].split(" ")
                ]
            )
        else:
            sales_invoice_links = ""
        ...
        transit_totals[row["transit_no"]]["purchase_invoice_amount"] += row[
            "purchase_invoice_amount"
        ]
        transit_totals[row["transit_no"]]["sales_invoice_amount"] += row[
    ...
    for i, row in enumerate(processed_data):
        ...
        if i == len(processed_data) - 1 or (
            processed_data[i + 1]["transit_no_link"].split(">")[1].split("<")[0]
            != current_transit_no
        ):
