...
class Analytics:
    def __init__(self, filters=None):
        ...
        if (
            self.filters.doc_type == "Payment Entry"
    ...
    def run(self, filters):
        ...
        if (
            filters.get("tree_type") == "Item"
            and filters.get("value_quantity") == "Quantity"
        ):
...
def convert_currency_columns(data, filters):
    ...
    to_date = datetime.strptime(
        filters.get("to_date") or frappe.utils.now(), "%Y-%m-%d"
    )
    ...
    for entry in data:
        for field in currency_fields:
            entry[field] = convert(
                entry.get(field, 0), from_currency, to_currency, to_date
            )
    ...
    if value_qty == "Value":
        for column in columns:
            if column["label"] not in columns_not_to_include and presentation_currency:
                column["label"] = (
                    f"{column['label']} (<strong>{presentation_currency}</strong>)"
                )
