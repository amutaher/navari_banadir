...
def get_total_quantity_for_customer(customer, filters):
    ...
    for item in sales_invoice_items + delivery_note_items:
        ...
        if filters.get("alternative_uom"):
            conversion_factor = get_conversion_factor(
                item["item_code"], filters["alternative_uom"]
            )
            qty /= conversion_factor

        total_qty += qty

    return_invoice_items = frappe.get_all(
        "Sales Invoice Item",
        filters={
            "parent": [
                "in",
    ...
    return_delivery_note_items = frappe.get_all(
        "Delivery Note Item",
        filters={
            "parent": [
                "in",
    ...
    for row in data:
        ...
        if row.get("indent") == 0:
            ...
            try:
                ...
                if (
                    hasattr(invoice_doc, "custom_stock_entry")
                    and invoice_doc.custom_stock_entry
                ):
                    stock_entries = [
                        entry.stock_entry for entry in invoice_doc.custom_stock_entry
                    ]

                    for stock_entry_name in stock_entries:
                        append_stock_entry_details(
                            updated_data, stock_entry_name, invoice_or_item
                        )

            except frappe.DoesNotExistError:
                frappe.log_error(
                    f"Sales Invoice {invoice_or_item} not found.",
                    "Stock Entry Fetch Error",
                )
            except Exception as e:
                frappe.log_error(
                    f"Error processing invoice {invoice_or_item}: {str(e)}",
                    "Stock Entry Fetch Error",
                )
    ...
    updated_data.append(
        {
            ...
            "posting_date": frappe.get_value(
                "Stock Entry", stock_entry_name, "posting_date"
            ),
            "indent": 1,
            "is_group": 1,
            "parent": parent_invoice,
            "cost_amount": frappe.get_value(
                "Stock Entry", stock_entry_name, "total_outgoing_value"
            ),
            "qty": frappe.get_value(
                "Stock Entry", stock_entry_name, "custom_total_quantity"
            ),
            "bolden": 1,
        }
    )
