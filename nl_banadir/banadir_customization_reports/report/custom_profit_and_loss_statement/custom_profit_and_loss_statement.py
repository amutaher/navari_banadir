...
def get_period_list(
    ...
    months_to_add = {"Yearly": 12, "Half-Yearly": 6, "Quarterly": 3, "Monthly": 1}[
        periodicity
    ]
    ...
    for i in range(cint(math.ceil(months / months_to_add))):
        ...
        if not ignore_fiscal_year:
            period.to_date_fiscal_year = get_fiscal_year(
                period.to_date, company=company
            )[0]
...
def get_fiscal_year_data(from_fiscal_year, to_fiscal_year):
    ...
    from_fy_dates = frappe.db.get_value(
        "Fiscal Year",
        from_fiscal_year,
        ["year_start_date", "year_end_date"],
        as_dict=True,
    )
    to_fy_dates = frappe.db.get_value(
        "Fiscal Year",
        to_fiscal_year,
        ["year_start_date", "year_end_date"],
        as_dict=True,
    ...
    if not from_fy_dates or not to_fy_dates:
        return {}

    # Now, fetch the min and max date within the selected range
    fiscal_year = frappe.db.sql(
        """
		SELECT MIN(year_start_date) AS year_start_date, 
			   MAX(year_end_date) AS year_end_date
		FROM `tabFiscal Year`
		WHERE year_start_date >= %(from_start)s AND year_end_date <= %(to_end)s
		""",
        {
            "from_start": from_fy_dates["year_start_date"],
            "to_end": to_fy_dates["year_end_date"],
        },
        as_dict=1,
    )
...
def validate_fiscal_year(fiscal_year, from_fiscal_year, to_fiscal_year):
    ...
    if getdate(fiscal_year.get("year_end_date")) < getdate(
...
def accumulate_values_into_parents(accounts, accounts_by_name, period_list):
    ...
    for d in reversed(accounts):
        if d.parent_account:
            for period in period_list:
                accounts_by_name[d.parent_account][period.key] = accounts_by_name[
                    d.parent_account
                ].get(period.key, 0.0) + d.get(period.key, 0.0)

            accounts_by_name[d.parent_account]["opening_balance"] = accounts_by_name[
                d.parent_account
            ].get("opening_balance", 0.0) + d.get("opening_balance", 0.0)


def prepare_data(
    accounts, balance_must_be, period_list, company_currency, accumulated_values
):
    data = []
...
def apply_additional_conditions(
    doctype, query, from_date, ignore_closing_entries, filters
):
    gl_entry = frappe.qb.DocType(doctype)
    ...
    if filters:
        ...
        if filters.get("include_default_book_entries"):
            ...
            query = query.where(
                (
                    gl_entry.finance_book.isin(
                        [cstr(filters.finance_book), cstr(company_fb), ""]
                    )
                )
    ...
    if accounting_dimensions:
        for dimension in accounting_dimensions:
            if filters.get(dimension.fieldname):
                ...
                query = query.where(
                    gl_entry[dimension.fieldname].isin(filters[dimension.fieldname])
                )
