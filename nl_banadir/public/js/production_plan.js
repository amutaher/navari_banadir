...
frappe.ui.form.on("Production Plan", {
  refresh: function (frm) {
    ...
    frm.add_custom_button(__("Split Items"), function () {
      ...
      if (has_split) {
        frappe.call({
          method:
            "nl_banadir.banadir_customization_reports.controllers.production_plan.split_po_items",
          args: {
  ...
  custom_split_subassembly_item: function (frm) {
    let has_subassembly_split = frm.doc.sub_assembly_items.some(
      (item) => item.custom_split_no > 1,
    );

    if (has_subassembly_split) {
      frappe.call({
        method:
          "nl_banadir.banadir_customization_reports.controllers.production_plan.split_prod_assembly",
        args: {
          production_plan: frm.doc.name,
        },
        callback: function (r) {
      ...
      frappe.msgprint(
        __("No sub-assembly items with a split number greater than 1."),
      );
    }
  },
});
