frappe.ui.form.on("Purchase Order Item", {
  item_code: function (frm, cdt, cdn) {
    ...
    if (child.custom_work_order) {
      frappe.call({
        method:
          "nl_banadir.banadir_customization_reports.controllers.purchase_order.get_qty_from_first_work_order",
        args: {
        ...
        callback: function (response) {
          if (response.message) {
            frappe.model.set_value(
              ...
              cdn,
              "fg_item_qty",
              response.message.work_order_qty,
            );
            frappe.model.set_value(
              cdt,
              cdn,
              "fg_item",
              response.message.upper_stock_items,
            );
          }
        },
      });
  ...
  refresh: function (frm) {
    frm.add_custom_button(
      ...
      function () {
        ...
        frappe.call({
          method:
            "nl_banadir.banadir_customization_reports.controllers.purchase_order.get_items_from_production_plan",
          args: {
            ...
            if (response.message) {
              ...
              items.forEach((item) => {
                ...
                frappe.model.set_value(
                  ...
                  child.name,
                  "production_plan",
                  frm.doc.custom_production_plan,
                );
              });
    ...
    if (frm.doc.is_subcontracted == 1) {
      frm.doc.items.forEach(function (child) {
        if (child.qty !== child.fg_item_qty) {
          frappe.msgprint(
            __(
              "The <b>quantity</b> must be the same as the <b>FG Item Quantity</b>. Please correct the values.",
            ),
          );
